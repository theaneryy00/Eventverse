...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_event'])) {
    ...
    if (empty($name) || empty($date)) {
        ...
        error_log("[ERROR] Event creation failed: Missing fields.", 3, "error_log.txt");
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['message_type'] = "error";
            error_log("[ERROR] Failed to insert event: " . $conn->error, 3, "error_log.txt");
        }
        $stmt->close();
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_event'])) {
    ...
    if (empty($name) || empty($date)) {
        $_SESSION['message'] = "All fields are required.";
        $_SESSION['message_type'] = "error";
        error_log("[ERROR] Event update failed: Missing fields.", 3, "error_log.txt");
    } elseif (!validate_event_name($name)) {
        $_SESSION['message'] = "Event name must be 1-100 characters and contain only letters, numbers, spaces, hyphens, apostrophes, and periods.";
        $_SESSION['message_type'] = "error";
    } elseif (!validate_event_date($date)) {
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['message_type'] = "error";
            error_log("[ERROR] Failed to update event ID $id: " . $conn->error, 3, "error_log.txt");
        }
        $stmt->close();
    }
    header("Location: manage_events.php");
...
if (!$events) {
    error_log("[ERROR] Failed to fetch events: " . $conn->error, 3, "error_log.txt");
}
?>

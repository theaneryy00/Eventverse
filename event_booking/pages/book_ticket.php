...
if (!$event_id) {
    ...
    error_log("[ERROR] Event ID missing in request.", 3, "error_log.txt");
...
if (!$stmt) {
    error_log("[ERROR] Failed to prepare event details query: " . $conn->error, 3, "error_log.txt");
}
...
if (!$result) {
    error_log("[ERROR] Failed to fetch booked seats: " . $conn->error, 3, "error_log.txt");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$ticket_category || !$seat_number) {
        ...
        error_log("[ERROR] Ticket booking failed: Missing fields.", 3, "error_log.txt");
    ...
    if (in_array($seat_number, $booked_seats)) {
        ...
        error_log("[ERROR] Seat $seat_number is already booked for event ID $event_id.", 3, "error_log.txt");
        header("Location: book_ticket.php?event_id=$event_id");
        exit();
    }
    ...
    if (!$stmt) {
        error_log("[ERROR] Failed to prepare ticket insert query: " . $conn->error, 3, "error_log.txt");
    }
    ...
    if ($stmt->execute()) {
        ...
        error_log("[ERROR] Failed to book ticket: " . $stmt->error, 3, "error_log.txt");
    }
    $stmt->close();
}
?>
